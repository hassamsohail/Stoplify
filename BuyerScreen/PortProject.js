...
export default function PostProject() {
  ...
  const [selectedOption, setSelectedOption] = useState('Large'); // State to manage selected option

  const handleRadioButtonPress = (option) => {
    setSelectedOption(option);
    console.log("Selected option:", option); // Log selected option to console
  };
  const [selectedOptionmonths, setSelectedOptionmonths] = useState('6months'); // State to manage selected option
  ...
  const [selectedOptionrate, setSelectedOptionrate] = useState('Hourly'); // State to manage selected option
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("This is useridof ", userId)
      } catch (error) {
